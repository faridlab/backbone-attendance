//! Domain policy for RosterEntry aggregate
//!
...
/// Domain policy for RosterEntry — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<RosterEntry>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type RosterEntryDomainPolicy = PermitAllPolicy<RosterEntry>;
