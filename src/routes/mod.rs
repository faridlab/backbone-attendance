...
/// Create stateless routes for the Attendance module.
///
...
///
/// ```ignore
/// let module = AttendanceModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::AttendanceModule) -> Router<()> {
    Router::new()
        .merge(create_attendance_routes(module.attendance_service.clone()))
        .merge(create_attendance_clock_routes(module.attendance_clock_service.clone()))
}

/// Read-only routes for the Attendance module — every entity mounted READ-ONLY (the guarded base).
///
...
/// service's invariants. Extend it: `create_readonly_attendance_routes(m).merge(my_validated_writes)`.
pub fn create_readonly_attendance_routes(module: &crate::AttendanceModule) -> Router<()> {
    Router::new()
        .merge(create_attendance_read_routes(module.attendance_service.clone()))
        .merge(create_attendance_clock_read_routes(module.attendance_clock_service.clone()))
}

/// Get all routes (stateless) for the Attendance module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes(module: &crate::AttendanceModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Attendance module.
///
...
pub fn create_combined_routes(module: &crate::AttendanceModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Attendance module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::AttendanceModule) -> Router<AppState> {
    Router::new()
