...
metaphor migration generate Invoice attendance
metaphor migration run
...
(`attendance` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed attendance          # run Rust seeders
metaphor migration generate-seeds attendance  # emit SQL seed files
```
